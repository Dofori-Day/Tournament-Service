...
- [x] Додати тести для `SecurityConfig`:
  - інтеграційний тест для перевірки, що `/auth/**` доступний без аутентифікації
  - що інші ендпоінти потребують аутентифікації
- [x] Додати тести для `WebConfig` (CORS-конфігурація)
- [x] Додати тести для `MySqlConfig`
- [x] Підвищити coverage до 80%+:
  - додати тести для репозиторіїв з H2 вбудованою БД або @JdbcTest
  - додати тести для граничних випадків у DTO-класах
- [x] Додати інтеграційний тест для `Dispatcher` (main-метод)
- [ ] Додати тести для валідації даних (якщо додасться `@Valid`)

---

## 9. Як запускати тести
